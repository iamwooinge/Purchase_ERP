...
<%@page import="claim.claimDTO" %>
<%@page import="claim.claimDAO" %>
...
<title>청구 등록</title>
   ...
   <script>
    ...
    // 테이블에서 품목 선택시, select element 변경 이벤트
...
<% //로그인 할 때 employee__no를 불러옴
    ...
    String sql = "select employee_name,team_code,dept_code from employee where employee_no = '"+employee_no+"'"; 
    ...
    if(team_code.equals("C")||dept_code.equals("1")){
       %>
       <script>
...
try{
    ...
    if(request.getParameter("claim_date") != null){
       claim_date = request.getParameter("claim_date");
       claim_no = claim_date.replace("-", "").replace("20","");
    }
    if(request.getParameter("product_no") != null){
       product_no = request.getParameter("product_no");
       claim_no = claim_no+"-"+product_no;
    }
...
// 주문번호, 품목별 개수 구해서 뒤에 붙임   
    String sql2 = "select count(*) from claim where product_no ='" + product_no + "'";
    ...
    if(rs2.next()){
       cnt = rs2.getInt(1)+1;
       if(cnt < 10){
          claim_no = claim_no + "0" + cnt;
       }else{
          claim_no = claim_no + cnt;
       }
    }
    
    claimDAO claimDAO = new claimDAO();
    int result = claimDAO.claim(claim_no, claim_date, product_no, employee_no, product_amount, product_standard, product_unit);
}catch(Exception e){
	   e.printStackTrace();
	 %>
	       <script>
...
<!-- 테이블 코드  -->
  ...
  <h2 id = "title">청구 등록</h2>

   

      <form action="./add_billing.jsp" method="post" id="claim">
         <td> <input type="date" name ="claim_date" style="display:none;"/></td>
      <script>
      var date = new Date();
         ...
         <thead>
            ...
            <tr>
              ...
              <select type="text" style="width:100%; height:100%; border:1px solid white;" id="inproname" name="product_no" onchange="changeProductSelect()">
                 ...
                 <td> <div class="search_btn_billing2"><button type="submit" id="save_billing" value="저장" >저장</button></div></td>
                
              <!--  <input type="submit" id="save_billing" value="저장" >저장</td>-->
            </tr>
      ...
      <table style="position:absolute; left:240px; top:300px;">
         <tr><td><div class="search_btn_billing2"><button id ="mrp" onclick = "showPopup1();" id="save_billing">MRP</button></div></a>&nbsp;&nbsp;&nbsp;</td></tr>
      </table>

<script type="text/javascript">
function showPopup1(){
    window.open("mrp.jsp","","width=1100, height=700, top=150, left=400");
}
